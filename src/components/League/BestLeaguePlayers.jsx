...
export default function BestLeaguePlayers({ competitionId }) {
  const theme = useTheme();
  const {competition, loading, error} = useCompetitions (competitionId);
  const { players, loading: playersLoading, error: playersError } = useLeaguePlayersScoreAndTrends({competitionId});
      ...
      <Accordion defaultExpanded>
        ...
        >
          <Typography sx={{ fontWeight: 600, fontSize: isMobile ? '18px' : '20px' }}>
            {competition.competitionname} Best Young Players ⭐️
          </Typography>
        </AccordionSummary>
