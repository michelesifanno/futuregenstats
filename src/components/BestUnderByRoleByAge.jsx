...
import { useBestPlayersByRoleByAge } from '../utils/useBestPlayersByRoleByAge';
...
export default function BestUnderByRoleByAge({role, ageCategory}) {

  const theme = useTheme();
  const { players, loading: playersLoading, error: playersError } = useBestPlayersByRoleByAge(role, ageCategory);
  ...
  return (
    <>
      {players.length === 0 ? (
        ...
        <TableContainer>
        <Table aria-label="best-players-by-role-by-age">
          <TableBody>
