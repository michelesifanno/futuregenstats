...
const fetchPlayerPerformance = async (playerId) => {
  const options = {
    ...
    params: {
      player_id: playerId,
      locale: 'UK',
      season_id: '2019'  // Cambiato per recuperare i dati della stagione 2022
    },
    headers: {
...
const insertPlayerPerformance = async (playerId, performanceData) => {
  ...
  try {
    const { data, error } = await supabase
      .from('players_performance_19')  // Inserimento nella tabella `players_performance_22`
